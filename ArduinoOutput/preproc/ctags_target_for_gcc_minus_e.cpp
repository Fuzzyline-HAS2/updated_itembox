...
void Puzzle(void)
{
    ...
    if (digitalRead(34) == 0x0) // 엔코더 스위치 눌렸을때
    {
        ...
        if (differenceValue == 0) // 정답일때
        {
            ...
            if (answerCnt >= modeValue[RANGE][ANSWER_CNT]) // 모든 정답을 맞추었을때
            {
                ...
                detachInterrupt(13); // 하드웨어 인터럽트 종료 -> 엔코더 사용 막기 위해
                detachInterrupt(15);
...
void RfidLoopInner()
{
  ...
  { // rfid 통신 가능한 상태인지 확인
    if (nfc[INPN532].startPassiveTargetIDDetection((0x00)))
...
void RfidLoopOutter()
{
  ...
  { // rfid 통신 가능한 상태인지 확인
    if (nfc[OUTPN532].startPassiveTargetIDDetection((0x00)))
    { // rfid에 tag 찍혔는지 확인용 //데이터 들어오면 uid정보 가져오기
...
void StartPuzzle()
{
  ...
  attachInterrupt(13, updateEncoder, 0x03); // 엔코더 하드웨어 인터럽트 활성화
